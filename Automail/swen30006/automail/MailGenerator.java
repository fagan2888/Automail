...
/**
 * This class generates the mails
 */
public class MailGenerator {
     ...
     */
    public MailGenerator(int mailToCreate, IMailPool mailPool, Integer seed, boolean fragile){
        if(seed != null){
    ...
    private int generateArrivalTime(){
        return 1 + random.nextInt(Clock.getInstance().getLastDeliveryTime());
    }
    ...
    public PriorityMailItem step(){
        ...
        if(this.allMail.containsKey(Clock.getInstance().getTime())){
            for(MailItem mailItem : allMail.get(Clock.getInstance().getTime())){
            	if (mailItem instanceof PriorityMailItem) priority = ((PriorityMailItem) mailItem);
                System.out.printf("T: %3d > new addToPool [%s]%n", Clock.getInstance().getTime(), mailItem.toString());
